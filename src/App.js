...
class App extends Component {
  ...
  runThisShit = persons => {
    axios
      ...
      .then(function(response) {
        ...
        console.log(list);
        this.setState({ persons: response.data }).bind(this);
      })
  ...
  renderItem(item) {
    ...
    const itemRows = [
      <tr onClick={clickCallback} key={"row-data-" + item}>
        <td>{item}</td>
      </tr>
    ];

    if (this.state.expandedRows.includes(item)) {
      itemRows.push(
        <tr key={"row-expanded-" + item}>
          <td>{item}</td>
        </tr>
      );
    }
  ...
  renderItem(item) {
    const clickCallback = () => this.handleRowClick(item);
    const closeClickCallBack = () => this.handleRadioClick(item);
    const itemRows = [
      <tr key={"row-data-" + item}>
        <td>
          <RadioButtonComponent onClick={closeClickCallBack} name={item} />
        </td>
        <td>{item}</td>
        <td>
          <RadioButtonComponent onClick={clickCallback} name={item} />
        </td>
      </tr>
    ];
  ...
  render() {
    ...
    // (function(response) {
    //   console.log("Is this running nig: " + JSON.stringify(response));
